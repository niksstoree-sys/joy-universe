"""
Cog Welcome System untuk JOY UNIVERSE.

Mendukung konfigurasi lengkap lewat Prefix Command (`!welcome ...`) dan
Slash Command (`/welcome ...`), termasuk Embed, Card (image), Button,
mention user, dan seluruh custom variable.
"""
...
logger = logging.getLogger("joyuniverse.cogs.welcome")
...
class Welcome(commands.GroupCog, name="welcome"):
    """Slash commands otomatis ter-grup di bawah `/welcome`."""

    def __init__(self, bot: JoyUniverse):
        self.bot = bot
        self.service = GreetingService(bot.db, "welcome_config")
    ...
    async def on_member_join(self, member: discord.Member):
        try:
            ...
            await send_greeting_message(channel, greeting, member, member.guild, kind="welcome")
        except Exception:
            logger.exception("Gagal mengirim welcome message di guild %s", member.guild.id)
    ...
    async def welcome_group(self, ctx: commands.Context):
        """Menampilkan ringkasan konfigurasi welcome server ini."""
        ...
        embed = JoyEmbed.info(
            ...
            f"**Mention User:** {'Ya' if greeting.mention_user else 'Tidak'}\n\n"
            f"Gunakan `!welcome test` untuk preview, atau `!help welcome` untuk daftar lengkap subcommand.",
            title=f"{emoji.settings} Konfigurasi Welcome",
        )
    ...
    async def welcome_toggle(self, ctx: commands.Context, state: str):
        ...
        await ctx.send(embed=await self._confirm(f"Welcome system sekarang **{state.upper()}**."))
    ...
    @app_commands.command(name="settings", description="Menampilkan ringkasan konfigurasi welcome.")
    async def welcome_settings_slash(self, interaction: discord.Interaction):
    ...
    @app_commands.command(name="toggle", description="Aktifkan/nonaktifkan welcome system.")
    @app_commands.describe(state="on untuk aktifkan, off untuk nonaktifkan")
    @_manage_guild_slash()
    async def welcome_toggle_slash(self, interaction: discord.Interaction, state: Literal["on", "off"]):
        await self.service.set_enabled(interaction.guild_id, state == "on")
        await interaction.response.send_message(embed=await self._confirm(f"Welcome system sekarang **{state.upper()}**."))

    @app_commands.command(name="channel", description="Set channel tujuan welcome message.")
    @_manage_guild_slash()
    async def welcome_channel_slash(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await interaction.response.send_message(embed=await self._confirm(f"Channel welcome diset ke {channel.mention}."))

    @app_commands.command(name="set", description="Set salah satu bagian teks welcome (title, description, dst).")
    ...
    @_manage_guild_slash()
    async def welcome_set_slash(self, interaction: discord.Interaction, field: app_commands.Choice[str], value: str):
        column = field.value
    ...
    @app_commands.command(name="mention", description="Toggle mention user di pesan welcome.")
    @_manage_guild_slash()
    async def welcome_mention_slash(self, interaction: discord.Interaction, state: Literal["on", "off"]):
        await self.service.set_field(interaction.guild_id, "mention_user", int(state == "on"))
        await interaction.response.send_message(embed=await self._confirm(f"Mention user: **{state.upper()}**."))

    @app_commands.command(name="timestamp", description="Toggle timestamp di embed welcome.")
    @_manage_guild_slash()
    async def welcome_timestamp_slash(self, interaction: discord.Interaction, state: Literal["on", "off"]):
    ...
    @app_commands.command(name="card", description="Toggle welcome card (image).")
    @_manage_guild_slash()
    async def welcome_card_slash(self, interaction: discord.Interaction, state: Literal["on", "off"]):
        await self.service.set_field(interaction.guild_id, "card_enabled", int(state == "on"))
        await interaction.response.send_message(embed=await self._confirm(f"Welcome card: **{state.upper()}**."))

    @app_commands.command(name="cardbackground", description="Set/hapus background custom untuk welcome card.")
    async def welcome_cardbackground_slash(self, interaction: discord.Interaction, url: str):
        value = None if url.lower() == "none" else url
    ...
    @app_commands.command(name="avatarposition", description="Posisi avatar di welcome card.")
    @app_commands.choices(position=[app_commands.Choice(name=p, value=p) for p in POSITION_CHOICES])
    @_manage_guild_slash()
    async def welcome_avatarposition_slash(self, interaction: discord.Interaction, position: app_commands.Choice[str]):
    ...
    @app_commands.command(name="textposition", description="Posisi teks di welcome card.")
    @app_commands.choices(position=[app_commands.Choice(name=p, value=p) for p in TEXT_POSITION_CHOICES])
    @_manage_guild_slash()
    async def welcome_textposition_slash(self, interaction: discord.Interaction, position: app_commands.Choice[str]):
    ...
    @app_commands.command(name="button", description="Tambah button link di pesan welcome.")
    @_manage_guild_slash()
    async def welcome_button_slash(self, interaction: discord.Interaction, label: str, url: str):
    ...
    @app_commands.command(name="removebutton", description="Hapus button dari pesan welcome.")
    @_manage_guild_slash()
    async def welcome_removebutton_slash(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=await self._confirm("Button welcome dihapus."))

    @app_commands.command(name="test", description="Kirim contoh preview welcome message di channel ini.")
    @_manage_guild_slash()
    async def welcome_test_slash(self, interaction: discord.Interaction):
        greeting = await self.service.get(interaction.guild_id)
        await interaction.response.defer()
        message = await send_greeting_message(interaction.channel, greeting, interaction.user, interaction.guild, kind="welcome")
        if message is None:
    ...
    @app_commands.command(name="reset", description="Reset seluruh konfigurasi welcome ke default.")
    @_manage_guild_slash()
    async def welcome_reset_slash(self, interaction: discord.Interaction):
        await self.service.reset(interaction.guild_id)
        await interaction.response.send_message(embed=await self._confirm("Konfigurasi welcome dikembalikan ke default."))

    @app_commands.command(name="variables", description="Menampilkan daftar custom variable yang tersedia.")
    async def welcome_variables_slash(self, interaction: discord.Interaction):
